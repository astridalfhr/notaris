...
<?php
// fallback supaya tampilan rapi walau data kosong
$settings = $settings ?? [];
$heroes = $heroes ?? [];     // dari site_hero
$featured = $featured ?? [];   // fallback slider jika heroes kosong
$latest = $latest ?? [];
    ...
    <div class="relative overflow-hidden">
      <div class="slides flex transition-transform duration-700 ease-in-out" style="transform: translateX(0%)"
        ...
        <?php if (!empty($slidesData)): ?>
          ...
          <?php endforeach; ?>
        <?php else: ?>
          <!-- Minimal fallback jika semua kosong -->
          <article class="min-w-full p-10 flex flex-col md:flex-row items-center gap-10">
...
<script>
  (function () {
    ...
    // URL relatif (aman HTTPS tanpa .env)
    const feedUrl = "news/feed";
    const showUrl = id => "news/show/" + id;
    ...
    // Pasang handler ke card SSR hanya kalau id valid
    ...
    async function openDetail(id) {
      ...
      try {
        ...
        // pastikan benar-benar JSON (bukan HTML login)
