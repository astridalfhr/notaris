...
class Kerja extends BaseController
{
    ...
    public function item(string $kategori, string $slug)
    {
        ...
        if (!isset($this->MENU[$kategori])) {
            return redirect()->to(site_url('admin/kerja'))->with('error', 'Kategori tidak dikenal.');
        }
        ...
        if (!$title) {
            return redirect()->to(site_url('admin/kerja'))->with('error', 'Item tidak ditemukan.');
        }
    ...
    public function upload()
    {
        ...
        return ($origin !== 'umum' && substr_count($origin, '/') === 1)
            ? redirect()->to(site_url("admin/kerja/{$origin}"))->with('success', 'Berkas diunggah.')
