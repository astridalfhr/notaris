...
{
    private array $allowedRoles = ['user', 'admin'];

    private function guard(): ?\CodeIgniter\HTTP\RedirectResponse
    {
        $role = strtolower((string) session('role'));
        if ($role !== 'admin') {
            return redirect()->to(site_url('admin'))->with('error', 'Akses ditolak. Hanya Admin yang dapat mengelola role.');
        }
        return null;
    }
    ...
    public function update()
    {
        ...
        if ($method !== 'post') {
            return redirect()->to(site_url('admin/roles'))->with('error', 'Metode tidak diizinkan.');
        }
        ...
        if ($targetId === $meId && $myRole === 'admin' && $newRole !== 'admin') {
        ...
        return redirect()->to(site_url('admin/roles'))->with('success', 'Role pengguna diperbarui: ' . ucfirst($newRole));
    }
